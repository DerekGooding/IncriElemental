...
public class InputManager
{
    ...
    public void MockClick() => _mockClick = true;

    public Point MousePosition { get { if (_mockMousePos.HasValue) return _mockMousePos.Value; return new Point((int)(_currentMouse.X / UiScale), (int)(_currentMouse.Y / UiScale)); } }
    public int ScrollDelta => _currentMouse.ScrollWheelValue - _lastMouse.ScrollWheelValue;

    public void Update() { _lastMouse = _currentMouse; _currentMouse = Mouse.GetState(); _lastKey = _currentKey; _currentKey = Keyboard.GetState(); }
    public void ClearMock() { _mockKeys.Clear(); _mockClick = false; }

    public bool IsLeftClick() => _mockClick || (_currentMouse.LeftButton == ButtonState.Pressed && _lastMouse.LeftButton == ButtonState.Released);
    public bool IsRightClick() => _currentMouse.RightButton == ButtonState.Pressed && _lastMouse.RightButton == ButtonState.Released;
    public bool IsKeyPressed(Keys k) => _mockKeys.Contains(k) || (_currentKey.IsKeyDown(k) && _lastKey.IsKeyUp(k));

    public void HandleHotkeys(GameEngine engine, Action<GameTab> setTab)
    {
        if (IsKeyPressed(Keys.Space)) engine.Focus();
        ...
        if (IsKeyPressed(Keys.P) || IsRightClick()) IsTooltipPinned = !IsTooltipPinned;
    }
    ...
    public void ProcessButtons(List<Button> buttons, GameTab currentTab, float scrollOffset)
    {
        ...
        foreach (var btn in buttons)
        {
            ...
            if (btn.Tab == currentTab || btn.Tab == GameTab.None) { btn.Update(MousePosition, offset); if (isClick) btn.CheckClick(MousePosition, offset); }
        }
    }
}
