...
public class AiModeSystem(GameEngine engine)
{
    ...
    public void HandleAiUpdate(GameTime gt, GraphicsDevice gd, string defPath, Action<GameTime> draw, Action exit, VisualManager vis, List<Button> btns, InputManager input, TutorialSystem tut)
    {
        ...
        while (_commandIndex < _commands.Count)
        {
            ...
            if (act == "focus") _engine.Focus();
                ...
                var oldTut = tut.IsActive; tut.IsActive = false; vis.ClearShake(); vis.ClearTransitions();
                UiMetadataTracker.Clear(); UiMetadataTracker.IsEnabled = true;
                _game?.RequestScreenshot(p);
                draw(gt); 
                SaveMetadata(p.Replace(".png", ".json"), btns, gt); 
                UiMetadataTracker.IsEnabled = false;
                tut.IsActive = oldTut;
                isBlocking = true;
            }
    ...
    private void SaveMetadata(string path, List<Button> buttons, GameTime gameTime)
    {
        var metadata = new { 
            ...
            Buttons = buttons.Where(b => b.IsVisible() && (b.Tab == _game?.GetCurrentTab() || b.Tab == GameTab.None)).Select(b => new { Text = b.Text, Subtitle = b.Subtitle, Tooltip = b.TooltipFunc?.Invoke() ?? "", Intent = b.Intent, Bounds = new { b.Bounds.X, b.Bounds.Y, b.Bounds.Width, b.Bounds.Height }, Tab = b.Tab.ToString() }).ToList(), 
