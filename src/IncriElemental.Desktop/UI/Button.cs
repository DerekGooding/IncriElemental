...
public class Button(Rectangle bounds, string text, Color color, Action onClick, Func<bool>? isVisible = null, string? subtitle = null, GameTab tab = GameTab.Void)
{
    ...
    public void Draw(SpriteBatch spriteBatch, SpriteFont? font, Texture2D pixel)
    {
        ...
        if (font != null)
        {
            ...
            var textPos = new Vector2(Bounds.Center.X - (textSize.X * scale) / 2, Bounds.Center.Y - (textSize.Y * scale) / 2 + yOffset);
            ...
            if (!string.IsNullOrEmpty(Subtitle))
            {
                ...
                var subPos = new Vector2(Bounds.Center.X - (font.MeasureString(Subtitle).X * subScale) / 2, textPos.Y + (textSize.Y * scale) - 2);
